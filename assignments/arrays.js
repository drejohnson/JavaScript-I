...
let recalled;
inventory
  .filter(car => car.id === 33)
  .map(car => recalled = car);

console.log(`Car 33 is a ${recalled.car_year} ${recalled.car_make} ${recalled.car_model}`);

// ==== Challenge 2 ====
// The dealer needs the information on the last car in their inventory.  What is the make and model of the last car in the inventory?  Log the make and model into the console.
let lastCar = 0;
lastCar = inventory.slice(-1)[lastCar]
...
carModels.map(car => carModelsSorted.push(car))
...
let oldCars = [];
carYears.filter(year => year > 2000).map(year => oldCars.push(year))
console.log(oldCars);
...
let BMWAndAudi = [];
inventory
  .filter(cars => cars.car_make === "BMW" || cars.car_make === "Audi")
  .map(cars => BMWAndAudi.push(cars))
